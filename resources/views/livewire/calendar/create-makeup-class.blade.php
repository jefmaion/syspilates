                            ...
                            <div class="card mt-3">
                                <div class="card-header p-2">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <span class="mx-1"><x-icons.calendar /><strong> {{ $class->origin->datetime->format('d/m/y') }}</strong></span> |
                                        <span class="mx-1"><x-icons.time /> {{ $class->origin->datetime->format('H\h') }}</span> |
                                        <span class="mx-1"><x-page.badge color="{{ $class->origin->status->color() }}">{{ $class->origin->status->label() }}</x-page.badge></span>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <div class="text-mutesd text-sm mb-2">
                                        <x-icons.modality /> {{ $class->origin->modality->name }} |
                                        <x-page.avatar size="xs" :user="$class->origin->instructor->user" /> {{ $class->origin->instructor->user->shortName ?? null }}
                                    </div>
